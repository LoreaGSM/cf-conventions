...
The permitted values (case-insensitive) are **`normal`** and **`octahedral`**.
...
The **`points_per_latitude`** and **`accumulated_points_per_latitude`** variables are equivalent; the latter is the cumulative sum of the former, and may ease user computations of latitudes and longitudes.
...
It must have a **`units`** attribute and may have other attributes to identify it as containing latitude coordinates (see Section 4.2).
The latitudes correspond to the zeroes of the Legendre polynomial of order _N_.
+
The data variable must have an integer-valued one-dimensional coordinate variable with **`standard_name`** attribute value **`reduced_gaussian_index`**.
This is a discrete index with a unique value to identify each gridpoint.
The **`reduced_gaussian_index`**, together with the numbers of longitude points for each latitude line and the **`latitude`** values, can be used to reconstruct the latitude and longitude of each of the gridpoints.
+
